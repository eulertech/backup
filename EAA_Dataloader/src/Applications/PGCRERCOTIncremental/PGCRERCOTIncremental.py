...
class PGCRERCOTIncremental(ApplicationBase):
    '''
    ...
    def __init__(self):
        ...
        Initial settings
        '''
        super(PGCRERCOTIncremental, self).__init__()
        ...
        try:
            ...
            for fileConfig in self.job["files"]:
                ...
                for zipFileName in zipFiles:  # eg cdr.00012328.0000000000000000.20170619.123601002.DAMHRLMPNP4183_csv.zip
                    ...
                    self.fileUtilities.UnzipUsing7z(self.localTempDirectory + "/raw/" + zipFileName.split("/")[1], self.localTempDirectory + "/output/")
                    FileUtilities.RemoveFileIfItExists(self.localTempDirectory + "/raw/" + zipFileName)  # delete the parent file
                    self.AddColumnSkipHeader(self.localTempDirectory + "/output/")  # Add column DSTFlag if it doesn't exist and skip header
                    self.PackFiles(self.localTempDirectory + "/output/", self.localTempDirectory + "/packed/")
        except:
    ...
    def PostLoadETL(self):
        ...
        if postLoadScriptTemplate is not None:
            sqlTableCreationScript = super(PGCRERCOTIncremental, self).BuildTableCreationScript(postLoadScriptTemplate)
    ...
    def Start(self, logger, moduleName, filelocs):
        ...
        try:
            ...
            maxModifiedDatetime, fileCount = self.ProcessFiles(lastModifiedDatetime)
            if fileCount > 0:
