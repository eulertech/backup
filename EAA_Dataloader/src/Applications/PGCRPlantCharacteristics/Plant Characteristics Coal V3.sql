...
--        END  AS gross_load_mw_change_ratio,
        CASE WHEN operating_time=0 OR operating_time IS NULL OR gross_load_mw=0 OR gross_load_mw IS NULL THEN 0 ELSE heat_input_mmbtu/(gross_load_mw * operating_time) END AS heat_efficiency,
...
SELECT a.*, b.am_facility_id_orispl, b.am_unit_id, b.flag_manually_mapped_by_aa as flag_manually_mapped, c.net_gen_multiplier from 
...
ALTER TABLE pgcr_dev.plant_characteristics_analysis ADD COLUMN net_gen_multiplier float;
ALTER TABLE pgcr_dev.plant_characteristics_analysis ADD COLUMN net_load_mw float;
ALTER TABLE pgcr_dev.plant_characteristics_analysis ADD COLUMN net_load_mw_lag float;
ALTER TABLE pgcr_dev.plant_characteristics_analysis ADD COLUMN flag_manually_mapped int;
...
SELECT state, year
       ...
       ,SUM(gross_load_mw * operating_time) AS gross_load_mw_denom_30
       ...
       ,count(gross_load_mw) as hours_30
INTO pgcr_dev.wtavg_heatrate_1
FROM pgcr_dev.plant_characteristics_no_load_blips
WHERE   gross_load_mw >= nameplatecapacity_mw*0.3
AND heat_efficiency >=5 AND heat_efficiency <=25
GROUP BY state, year
         ,facility_name, facility_id_orispl, flag_manually_mapped
         ,unit_id
         ,unit_type
...
SELECT state, year
       ...
       ,MAX(net_gen_multiplier) as net_gen_multiplier
       ,SUM(gross_load_mw * operating_time) AS gross_load_mw_denom_0
       ,SUM(heat_input_mmbtu) AS heat_input_mmbtu_num_0
       , case when SUM(gross_load_mw * operating_time) <>0 then SUM(heat_input_mmbtu)/SUM(gross_load_mw * operating_time)
              else NULL
         end heat_rate_above_0
       ,count(gross_load_mw) as hours_0
INTO pgcr_dev.wtavg_heatrate_2
FROM pgcr_dev.plant_characteristics_no_load_blips
WHERE heat_efficiency >=5 AND heat_efficiency <=25
GROUP BY state, year
         ,facility_name, facility_id_orispl, flag_manually_mapped
         ,unit_id
         ,unit_type
...
AS
SELECT b.*, a.gross_load_mw_denom_30,a.heat_input_mmbtu_num_30, a.heat_rate_above_30perc, a.hours_30
...
SELECT state, year
       ...
       ,fuel_type_primary
       ,fuel_type_secondary
       ,SUM(gross_load_mw * operating_time) AS gross_load_mw_denom
       ,SUM(heat_input_mmbtu) AS heat_input_mmbtu_num
       , case when SUM(gross_load_mw * operating_time) <>0 then SUM(heat_input_mmbtu)/SUM(gross_load_mw * operating_time)
              else NULL
         end full_load_heatrate80
       ,count(gross_load_mw) as hours_80
INTO pgcr_dev.full_load_heatrate80
FROM pgcr_dev.plant_characteristics_no_load_blips
WHERE   gross_load_mw >= nameplatecapacity_mw*0.8
AND heat_efficiency >=5 AND heat_efficiency <=25
GROUP BY state, year
         ,facility_name, facility_id_orispl, flag_manually_mapped
         ,unit_id
...
SELECT state, year
       ...
       ,count(gross_load_mw) as hours_85
INTO pgcr_dev.full_load_heatrate85
FROM pgcr_dev.plant_characteristics_no_load_blips
WHERE   gross_load_mw >= nameplatecapacity_mw*0.85
AND heat_efficiency >=5 AND heat_efficiency <=25
GROUP BY state, year
         ,facility_name, facility_id_orispl, flag_manually_mapped
         ,unit_id
...
SELECT cast(DATE_PART(year,DATE::DATE) as int) AS year
       ...
       ,case when DATE_PART(year,DATE::DATE)<2017 then 8760 else max(DATE_PART(mon,DATE::DATE)) * 8760 / 12 end hours
...
SET capacity_factor = net_load_mw / (nameplatecapacity_mw * hours)
WHERE nameplatecapacity_mw<>0;

ALTER TABLE pgcr_dev.capacity_factor_by_year ADD COLUMN gross_capacity_factor FLOAT;
UPDATE pgcr_dev.capacity_factor_by_year 
SET gross_capacity_factor = gross_load_mw / (nameplatecapacity_mw * hours)
WHERE nameplatecapacity_mw<>0;
...
select COALESCE(h.state,c.state) as state, COALESCE(h.year,c.year) as year
       ...
       ,h.net_gen_multiplier
       ,CASE WHEN c.gross_capacity_factor < 0.1 THEN h.heat_rate_above_30perc
             WHEN c.gross_capacity_factor >= 0.1 THEN h.heat_rate_above_30perc * h.net_gen_multiplier
             WHEN c.gross_capacity_factor IS NULL THEN h.heat_rate_above_30perc * h.net_gen_multiplier
        END heat_rate_above_30perc
       ,h.hours_30
       ,CASE WHEN c.gross_capacity_factor < 0.1 THEN h.heat_rate_above_0
             WHEN c.gross_capacity_factor >= 0.1 THEN h.heat_rate_above_0 * h.net_gen_multiplier
             WHEN c.gross_capacity_factor IS NULL THEN h.heat_rate_above_0 * h.net_gen_multiplier
        END heat_rate_above_0
       ,h.hours_0
       ,CASE WHEN c.gross_capacity_factor < 0.1 THEN h.full_load_heatrate80
             WHEN c.gross_capacity_factor >= 0.1 THEN h.full_load_heatrate80 * h.net_gen_multiplier
             WHEN c.gross_capacity_factor IS NULL THEN h.full_load_heatrate80 * h.net_gen_multiplier
        END full_load_heatrate80
       ,h.hours_80
       ,CASE WHEN c.gross_capacity_factor < 0.1 THEN h.full_load_heatrate85
             WHEN c.gross_capacity_factor >= 0.1 THEN h.full_load_heatrate85 * h.net_gen_multiplier
             WHEN c.gross_capacity_factor IS NULL THEN h.full_load_heatrate85 * h.net_gen_multiplier
        END full_load_heatrate85
       ,h.hours_85
       ,c.capacity_factor
