# Stability selection in the spirit of Meinshausen&Buhlman Input: df:
# dataframe with target at first column and rest after nboostrap :
# number of bootstrap Output: dataframe with two columns: varnames,
# coeficient 
# Liang Kuang, 04/19/2017

EaaRandomLASSO <- function(df, nbootstrap = 200, alpha = 0.2, allfeatures = TRUE, 
    sorted = TRUE) {
    ...
    for (i in seq(nbootstrap)) {
        ...
        # run the randomized lasso on each sample and check which variables are
        # selected
        x.train <- xs[i1, ]
        y.train <- y[i1]
    ...
    # normalize frequence in [0,1] the final stability score is the maximum
    # frequency over the steps
