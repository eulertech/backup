...
###Parameters identifcation problem in supply-demand systems 

Market prices are determined by the interaction of both demand and supply ; demand cannot be correctly estimated without also accounting for variation in price and quantity data attributable to both demand-side and supply-side variables **simultaneously**.  

The complexity of the system requires the collection of data on all the relevant demand-determining variables and all the relevant supply-determining variables. 

When a variable is determined by a system of equations, it is said to be an **endogenous variable** in that system. Most of the firms in the market place are price-takers, i.e. they must accept the price of the product as it is determined by market forces in a system of demand and supply equations. 

For *price-setting* firms, price is not determined by  a system of demand and supply equations. In this case, price is an exogenous variable because it is not determined within the system. 

The distinction between price-taking firms and price-setting firms plays an important role in determining how the parameters of an empirical demand function must be estimated in order to obtain estimates that are not biased. 

The fundamental difficulty in estimating and forecasting industry demand for price-taking firms arises because the observed quantity and price data used in a regression analysis of demand are determined simultaneously by the intersection of demand and supply. 
**The simultaneity problem** in estimating industry demand arises because variation in observed values of market quantity and price is *simultaneously* determined by changes in demand and supply. 

Therefore, **the observed variation in equilibrium quantity and price is caused by all the factors that can shift either demand or supply**. 


To understand the nature of the simultaneity problem, consider the following simple model of industry demand and supply curves for gasoline: 

$$
Demand:\ Q = a + bP + cM + \epsilon_{d} \\
Supply:\ Q = h + kP + lP_{l} + \epsilon_{s}
$$

Where, 

+ $M$ is the exogenous variable that shifts only demand; and
+ $P_{l}$ is the exogenous variable that shifts only supply. 

Solving the system by setting demand = supply yields the reduced-form equations of the system where the endogenous variables, the equilibrium values for $P$ and $Q$, are determined by the exogenous variables and the random error terms: 

$$
P_{e} = f(M, P_{l}, \epsilon_{d}, \epsilon_{s}) \ and \ Q_{e} = f(M, P_{l}, \epsilon_{d}, \epsilon_{s})
$$
